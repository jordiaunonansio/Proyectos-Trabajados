...
with open('archive/written_name_test_v2.csv') as f:
...
output_folder = 'archive/test_lletres'
...
for image_name in labeldict.keys():
    image_path = 'archive/test_v2/test/' + image_name
    lletresTest, nlletresTest = extreu_lletres(image_path)
    nlletresLabel = len(labeldict[image_name])

    if nlletresTest == nlletresLabel:
        if labeldict[image_name].isalpha(): # if the label is only letters
            for i in range(nlletresTest):
                lletraTest = lletresTest[i][0]
                lletraLabel = labeldict[image_name][i]

                # si hi ha menys de 500 mostres de la lletra
                if lletrescount[lletraLabel] <= 300:

                    lletra_path = 'archive/test_lletres/' + lletraLabel + str(lletrescount[lletraLabel]) +".png"
                    cv2.imwrite(lletra_path, cv2.cvtColor(lletraTest, cv2.COLOR_BGR2RGB))
                ...
                if all(value >= 299 for value in lletrescount.values()):
                    # finish execution
                    print("Finished")
                    break
